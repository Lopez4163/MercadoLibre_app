...
const BILLING_RESUME_RATE_LIMIT = {
  limit: 10,
  windowMs: 60_000,
};
...
export async function POST(request: NextRequest) {
  ...
  try {
    rateLimitDecision = await consumeRateLimit({
      key: buildRateLimitKey("api_billing_resume_post", userId, getRequestIp(request)),
      limit: BILLING_RESUME_RATE_LIMIT.limit,
      windowMs: BILLING_RESUME_RATE_LIMIT.windowMs,
    });
  } catch (error) {
    ...
    return NextResponse.json(
      {
        ok: false,
        error: "rate_limited",
        message: "Too many billing resume requests. Please wait and try again.",
      },
      {
        status: 429,
  ...
  try {
    ...
    return NextResponse.json(
      { ok: false, error: "stripe_resume_failed" },
